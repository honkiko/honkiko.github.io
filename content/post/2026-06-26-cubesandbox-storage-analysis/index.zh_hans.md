...
- 多个微虚机的系统盘，会产生重复的page cache吗？

- 容器镜像是保存在宿主机上还是微虚机里面？

- 容器的可写层是怎么实现的？ 

- 容器可写层的内容如何保存为快照(snapshot)？ 从快照启动sandbox时如何让容器看到可写层内容的？
...
以sandbox id 4d656bd35c9848f7abce297cd7cab6d2 为例，可以通过如下命令获取其微虚机的vm.info：
...
这里面指定了两个empty_dir的volume。 一个作为容器的可写层， 另一个类似k8s里面的emptyDir本地临时存储卷。后面章节会详细介绍它们的实现原理。

执行命令 cubecli cubebox create req.json 或
...
1. 微虚机内的文件系统(ext4)访问数据时，不走block I/O请求(bio)路径，直接读内存，性能更好

2. 微虚机内绕过page cache，避免了额外的page cache内存开销
...
注意这个virtiofs配置里面的`backendfs_config.cache`值为2。 不同取值的含义：

| cache 值 | 策略名称   | 核心行为                                | 一致性 / 性能权衡 |
| ------- | ------ | ----------------------------------- | ---------- |
| 0       | none   | 禁用 Guest 缓存，每次 IO 都从 Host 获取        | 一致性最佳，性能最低 |
| 1       | auto   | 元数据缓存 1 秒后过期，数据采用 close-to-open 一致性 | 平衡，默认推荐    |
| 2       | always | 所有内容永久缓存，永不主动失效                     | 性能最佳，一致性最差 |
...
类似 `1Gi/0/16ab3912-6b41-474d-b971-012d95bfd917`  这类文件，就是微虚机virio-blk块设备的raw image文件，其内容一开始是一个空的ext2文件系统。同样可以通过mount -o loop挂载到宿主机本地目录来查看其内容。 
...
overlayfs的lowerdir是通过virtiofs挂载到微虚机的那4个容器镜像layer目录 (/run/cube-containers/shared/containers 目录下)

overlayfs的upperdir  `/run/blk-cube/vda/disk/b48b9c87427847feb4595394a187d46b/upper`  就磁盘vda挂载点 `/run/blk-cube/vda/` 下的子目录
...
`cubebox_os_image/rfs-xxxx`  路径下的 .ext4文件比较大， 是容器镜像的内容打包为一个ext4文件系统的raw image。这个文件也可以通过`mount -o loop` 挂载到宿主机本地目录来查看其内容， 等价于容器镜像各layer叠加以后的文件系统视图。
 ...
 cubebox_os_image/rfs-xxxx` 路径下的 .vm文件是内核二进制文件。 这个.vm文件正常情况下，跟cubetoolbox/cube-kernel-scf/vmlinux 内容是完全一致的。 但考虑到打完一个快照后， 又更换了 cubetoolbox/cube-kernel-scf/vmlinux ， 那么从快照拉起sandbox时，如果使用 cubetoolbox/cube-kernel-scf/vmlinux， 就会出现内核符号表对不上。因此打快照时，需要把微虚机的内核二进制文件也保存一份到快照里面。
...
容器镜像被打包成一个ext4文件系统的raw image文件，作为虚拟块设备pmem1挂载到微虚机。
...
|                  | 从镜像创建sandbox                    | 从模板创建sandbox                           |
| ---------------- | ------------------------------- | -------------------------------------- |
| 宿主机上的容器镜像        | 每个layer一个子目录，跟containerd的默认机制一致 | 各layer合并后的视图，打包为一个ext4文件系统的raw image文件 |
| 容器镜像如何从宿主机传递到微虚机 | virtiofs (cache=always)         | pmem块设备（DAX）                           |
| 微虚机内page cache   | 会产生重复page cache                 | 无page cache开销                          |

按E2B协议，sandbox必须从模板创建。 但使用cubesandbox的场景中， 不排除需要直接从镜像创建sandbox，不经过模板快照。  在这个场景下，容器镜像如果采用pmem方式，I/O性能和内存开销都更优， 缺点是容器镜像需要多一个打包为raw image的步骤。
